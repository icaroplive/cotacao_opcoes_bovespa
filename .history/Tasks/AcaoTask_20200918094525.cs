    ...
    public class AcaoTask : IAcaoTask {
        ...
        public async Task<bool> sincronizarOpcoes () {
            ...
            foreach (var ativo in ativos) {
                ...
                foreach (var opcao in listaOpcoes.Options) {
                    var op = _context.opcao.Where (c => (c.acao.Codigo + c.acao.Numero + c.serie.Descricao + c.Descricao) == opcao.Symbol).FirstOrDefault ();
                    if (op != null) {
