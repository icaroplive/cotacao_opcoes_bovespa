...
namespace carteiraAcoes.Controllers {
    ...
    public class DashController : ControllerBase {
        ...
        public async Task<ActionResult<IEnumerable<MovimentoAcao>>> Getdash () {
            ...
            var x = await (from c in _context.movimentoAcao group c by new { c.AcaoId, c.DataVenda } into grp select new MovimentoAcao { Quantidade = grp.Sum (c => c.Quantidade) }).Include(c => c.AcaoId).ToListAsync ();
             return x;
            //return null;
