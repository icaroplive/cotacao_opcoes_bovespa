...
namespace carteiraAcoes.Controllers {
    ...
    public class DashController : ControllerBase {
        ...
        public async Task<ActionResult<IEnumerable<Dash>>> Getdash () {
            ...
            select new Dash { Acao = grp.Select(c => c.Acao).FirstOrDefault(), Quantidade = grp.Sum (c => c.Quantidade) , Lucro = grp.Sum(c => c.Quantidade) * (grp.Sum( c => c.ValorVenda) - grp.Sum(c => c.ValorVenda))  }
