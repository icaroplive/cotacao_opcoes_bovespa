...
namespace carteiraAcoes.Controllers {
    ...
    public class DashController : ControllerBase {
        ...
        public async Task<ActionResult<IEnumerable<Dash>>> Getdash () {
            ...
            var ew = _context.movimentoAcao.Where(c => c.Vendido).AsEnumerable ().GroupBy (c => new { c.AcaoId })
                .Select (c => new Dash() { Quantidade = c.Sum(c => c.Quantidade)}).Include(c => c.AcaoId).ToList();
